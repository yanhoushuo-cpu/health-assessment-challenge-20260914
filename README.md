...
**在线演示已恢复（2026-09-17）：** [青禾健康评估](https://uvyxrpacfugt.sealoshzh.site/)。本次运行窗口为北京时间 **2026-09-17 09:42:47—15:42:47**。已恢复数据库备份并重新验证历史会话、免费/付费字段、完整测评和模拟支付；浏览器完整报告可打开。到期自动停止，不能将此链接视为永久托管。候选人：鄢厚硕。详情见[交付记录](docs/delivery.md)。
...
**一次用户实际否决与修正（2026-09-15）：**AI 原先选择 Vercel + Supabase，用户指出国内网络与平台操作复杂度的问题，明确要求“环境尽量都改成国内的”。因此停止海外业务上线，改为国内云服务器可运行的 Next.js + Prisma/PostgreSQL，并增加离线镜像交付、真实容器支付和容器重建后持久化验证。理由是原题允许 Prisma + PostgreSQL，海外托管品牌并非硬性要求；评审可访问性与操作成本应纳入工程选择。这是对话中真实发生的用户判断，不声称用户亲自做过下面的代码审查。
