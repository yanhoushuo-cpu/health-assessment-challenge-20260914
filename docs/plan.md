...
- [x] 1. Foundation and domain: initialize npm/TypeScript/Vitest, write failing numeric boundary and algorithm tests, implement pure validated calculateHealth(input, now), verify test:unit.
- [x] 2. Persistence and HTTP: define Prisma schema/migration, launch PostgreSQL, write failing HTTP integration cases, implement session/auth, step version CAS, completion transaction, result DTO and payment lock/idempotency. Verify real database tests.
- [x] 3. Funnel: write browser journey first, implement responsive progressive questions, recovery, validation, results and accessible simulated paywall; run Playwright against real server.
